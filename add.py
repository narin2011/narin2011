...
Coded by a kid- github.com/narin2011
...
def banner():
    ...
    print(f'{lg}Version: {w}3.0{lg} | Author: {w}vnnarin12{rs}')
...
for a in accounts:
    ...
    clnt = TelegramClient(f'sessions/{phn}', 26293654, '8a8fe41a531234eb36a0defcbc247adb')
    clnt.connect()
    banned = []
...
for acc in to_use:
    stop = index + 60
    c = TelegramClient(f'sessions/{acc[0]}', 26293654 , '8a8fe41a531234eb36a0defcbc247adb')
    ...
    for user in members[index:stop]:
        ...
        try:
            if choice == 0:
                ...
                c(AddChatUserRequest(target_details.id, user, 3000))
